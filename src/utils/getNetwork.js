...
export const getNetworkIdentifier = (network) => {
  ...
  const selectedNetwork = (Feel.constants.MAINNET_NETHASH === network.networks.F39.nethash
    && networks.mainnet)
    || (Feel.constants.TESTNET_NETHASH === network.networks.F39.nethash && networks.testnet)
    || getNetwork(networkName);
  return !selectedNetwork.custom
    ? selectedNetwork.name.toLowerCase()
    : network.networks.F39.nethash;
};
...
export const getNetworkNameBasedOnNethash = (network, token = 'F39') => {

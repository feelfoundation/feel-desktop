import { validateAddress, validateF39PublicKey, validateAmountFormat } from './validators';
...
describe('Validate Address', () => {
  it('Should return -1 if empty adress', () => {
    expect(validateAddress('F39', '', networks.testnet.code)).toBe(-1);
    expect(validateAddress('BTC', '')).toBe(-1);
  });

  it('Should validate F39 address', () => {
    expect(validateAddress('F39', accounts.genesis.address)).toBe(0);
    expect(validateAddress('F39', '12345')).toBe(1);
  });
...
describe('Validate Public Key', () => {
  ...
  it('Should return 0 if public key is valid', () => {
    expect(validateF39PublicKey(accounts.genesis.publicKey)).toBe(0);
  });

  it('Should return 1 if public key is invalid', () => {
    expect(validateF39PublicKey(invalidPublicKey)).toBe(1);
  });
});

describe('Validate Amount Format', () => {
  const errors = {
    INVALID: i18n.t('Provide a correct amount of {{token}}', { token: 'F39' }),

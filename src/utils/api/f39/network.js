  ...
  if (network.name && (!apiClients[network.name] || network.name === networks.customNode.name)) {
    const { nethash, nodes } = {
      ...
      [networks.customNode.name]: {
        nethash: network.networks[tokenMap.F39.key] && network.networks[tokenMap.F39.key].nethash,
        nodes: [network.networks[tokenMap.F39.key] && network.networks[tokenMap.F39.key].nodeUrl],
      },
    }[network.name] || {};

...
describe('Utils: Account API', () => {
  ...
  describe('getAccount', () => {
    it('should return a promise that is resolved when feelAPIClient.getAccount() calls its callback with data.success == true', () => {
      ...
      return expect(requestPromise).to.eventually.eql({ ...account, serverPublicKey: publicKey, token: 'F39' });
    });
    ...
    it('should return a promise that is resolved even when feelAPIClient.getAccount() calls its callback with data.success == false and "Account not found"', async () => {
      ...
      expect(await getAccount({ passphrase, network })).to.eql({ ...account, token: 'F39', publicKey });
    });

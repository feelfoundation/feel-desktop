...
describe('FormBtc', () => {
  ...
  describe('should work with props.token BTC', () => {
    ...
    it('should update transaction priority fee when "High" is selected', async () => {
      ...
      expect(wrapper.find('div.transaction-priority')).toIncludeText(fromRawF39(transactionBaseFees.Low * mockFeeFactor));
      wrapper.find('button.option-High').simulate('click');
      await flushPromises();
      expect(wrapper.find('div.transaction-priority')).toIncludeText(fromRawF39(transactionBaseFees.High * mockFeeFactor));
    });
    ...
    it('should allow to set entire balance', async () => {
      ...
      wrapper.update();
      await flushPromises();
      expect(wrapper.find('.amount input').prop('value')).toEqual(fromRawF39(balance - (transactionBaseFees.Low * mockFeeFactor)));
    });
  });
});

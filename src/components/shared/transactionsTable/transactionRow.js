...
const TransactionRow = ({ data, className, t }) => (
  <DialogLink
    ...
    data={{ transactionId: data.id, token: 'F39' }}
  >
    ...
    <span className={grid['col-xs-2']}>
      <DateTimeFromTimestamp time={data.timestamp * 1000} token={tokenMap.F39.key} />
    </span>
    <span className={`${grid['col-xs-3']} ${grid['col-md-2']} ${styles.amount}`}>
      <FeelAmount val={data.amount} token={tokenMap.F39.key} />
      <span className={`${styles.fee} hideOnLargeViewPort`}><FeelAmount val={data.fee} token={tokenMap.F39.key} /></span>
    </span>
    <span className={`${grid['col-md-1']} ${styles.transactionFeeCell}`}>
      <Tooltip
        ...
        content={<FeelAmount val={data.fee} token={tokenMap.F39.key} />}
        size="s"
      >

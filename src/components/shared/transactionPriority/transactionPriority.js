...
import { toRawF39, fromRawF39 } from '../../../utils/f39';
...
const getRelevantPriorityOptions = (options, token) =>
  ...
  || (index === CUSTOM_FEE_INDEX && token === tokenMap.F39.key));

const isCustomFeeValid = (value, hardCap, minFee) => {
  if (!value) return false;
  const rawValue = toRawF39(parseFloat(value));

  if (rawValue > hardCap) {
    return false;
  }

  if (rawValue < toRawF39(minFee)) {
    return false;
  }
          ...
          isLoading ? (
            ...
            : (isCustom && !showEditIcon ? (
              <Input
                ...
                feedback={`fee must be between ${minFee} and ${fromRawF39(hardCap)}`}
              />
            ) : (

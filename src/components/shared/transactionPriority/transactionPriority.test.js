...
describe('TransactionPriority', () => {
  ...
  it('renders custom fee option only when props.token is f39', () => {
    expect(wrapper).not.toContainMatchingElement('.option-Custom');
    wrapper.setProps({ ...props, token: tokenMap.F39.key });
    expect(wrapper).toContainMatchingElement('.option-Custom');
  });

  it('renders custom fee option with input when props.token is f39', () => {
    expect(wrapper).not.toContainMatchingElement('.custom-fee-input');
    wrapper.setProps({ ...props, token: tokenMap.F39.key, selectedPriority: 3 });
